...
class Tree
  ...
  def height(value, node = self.root, counter = 0)
    return nil if node.nil?

    counter == 0 ? pointer = find(value) : pointer = node

    if !pointer.left.nil?
      height(value, pointer.left, counter += 1)
    elsif !pointer.right.nil?
      height(value, pointer.right, counter += 1)
    else
      return counter
    end
  end
  ...
  def balanced?
    # check the depth of every leaf
    # if the difference is not greater than one, return true
    # if the difference is greater than one, return false
...
bst = Tree.new(array)
bst.pretty_print
bst.preorder { |i| i.data * 2 }
bst.level_order { |i| i.data * 2 }
